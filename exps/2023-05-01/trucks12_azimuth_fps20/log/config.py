...
azimuth_pipeline = [
    ...
    dict(type='RandomFlip', flip_ratio=0.0),
    dict(
        type='Normalize', mean=[0, 0, 0], std=[6000, 6000, 6000], to_rgb=True),
    dict(type='DefaultFormatBundle'),
    dict(type='Collect', keys=['img'])
]
range_pipeline = [
...
trainset = dict(
    ...
    pipelines=dict(
        ...
        azimuth_static=[
            dict(
                type='LoadFromNumpyArray', force_float32=True, force_rgb=True),
            dict(type='Resize', img_scale=(416, 416), keep_ratio=False),
            dict(type='RandomFlip', flip_ratio=0.0),
            dict(
                type='Normalize',
...
work_dir = '/work/csamplawski_umass_edu/src/mmtracking/exps/2023-05-01/trucks12_azimuth/log'
backbone_cfg = dict(
    type='YOLOv7',
    weights=
...
adapter_cfgs = dict({
    ('azimuth_static', 'node_1'):
    dict(
        type='ConvAdapter',
        interpolate_size=(28, 20),
        interpolate_fn='avgpool'),
    ('azimuth_static', 'node_2'):
    dict(
        type='ConvAdapter',
        interpolate_size=(28, 20),
        interpolate_fn='avgpool'),
    ('azimuth_static', 'node_3'):
    dict(
        type='ConvAdapter',
        interpolate_size=(28, 20),
        interpolate_fn='avgpool'),
    ('azimuth_static', 'node_4'):
    dict(
        type='ConvAdapter',
        interpolate_size=(28, 20),
        interpolate_fn='avgpool')
            ...
            pipelines=dict(
                ...
                azimuth_static=[
                    dict(
                        ...
                        force_rgb=True),
                    dict(
                        type='Resize', img_scale=(416, 416), keep_ratio=False),
                    dict(type='RandomFlip', flip_ratio=0.0),
                    dict(
                        type='Normalize',
                        mean=[0, 0, 0],
