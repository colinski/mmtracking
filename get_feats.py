...
def process_file(hdf5_fname, node_num):
    ...
    out_file = h5py.File(base_path  + '/zed_r50_16.hdf5', 'w')
    f = h5py.File(hdf5_fname, 'r')
    timesteps = [k for k in f.keys()]
    for i in trange(len(timesteps)):
        ts = timesteps[i]
        code = f[ts][f'node_{node_num}']['zed_camera_left'][:]
...
data_root = 'data/mmm/2022-09-01/trucks1_lightsT_obstaclesF/'
for node_num in [1,2,3,4]:
    for split in ['train', 'test']:
        print(node_num, split)
        hdf5_fname= f'{data_root}/{split}/node_{node_num}/zed.hdf5'
